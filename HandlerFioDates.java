...
class HandlerFioDates {
    ...
    // Функция принимает строковые данные от пользователя и парсит их по пробелам на
    // выходе массив строк
    public String[] inputFioDate() throws DateIncorrectException {
        System.out.println("Необходимо ввести в одну строку данные пользователя");
        System.out.println(
                "Фамилия, имя, отчество, дата рождения(формат 01.01.2001), номер телефона (формат +79123456781), пол (формат f/m)");
    ...
    // Функция определяет является ли строка именем(Фамилией или Отчеством) не
    // взирая на раскладку(поддерживаемый язык РУС/EN)
        ...
        if ((str.codePointAt(0) >= 65 && str.codePointAt(0) <= 90)) { // английские имена раскладка EN
            for (int i = 1; i < len; i++)
                if (!(str.codePointAt(i) >= 97 && str.codePointAt(i) <= 122))
                    flag = false;
        } else if (str.codePointAt(0) >= 1040 && str.codePointAt(0) <= 1071) { // русские имена раскладка РУС
            for (int i = 1; i < len; i++)
    ...
    public boolean isTeltphoneNumber(String str) {
        ...
        if (str.length() > 12 || str.length() < 11)
            return false;
        else if (str.length() == 12 && str.substring(0, 2).equals("+7"))
            return isDigit(str.substring(1, 12));
    ...
    // Функция определяет удовлетворяют ли элементы массива[3] условия дата месяц
    // год корректность по отношению к текущей дате
    public boolean isCurentDate(String[] arrayOfString) {
        boolean flag = true;
        LocalDate date = LocalDate.now();
        if (!(Integer.parseInt(arrayOfString[0]) > 0 && Integer.parseInt(arrayOfString[0]) < 32))
            flag = false;
        if (!(Integer.parseInt(arrayOfString[1]) > 0 && Integer.parseInt(arrayOfString[1]) < 13))
            flag = false;
        ...
        if (Integer.parseInt(arrayOfString[2]) == date.getYear())
            if (Integer.parseInt(arrayOfString[1]) > date.getMonthValue())
                return false;
            else if (Integer.parseInt(arrayOfString[1]) == date.getMonthValue())
                if (Integer.parseInt(arrayOfString[0]) > date.getDayOfMonth())
                    return false;
        return flag;
    }
    ...
    public boolean isDateOfBirth(String str) {
        ...
        if (arrayOfString.length != 3)
            return false;
        else if (arrayOfString[0].length() == 2 && arrayOfString[1].length() == 2 && arrayOfString[2].length() == 4) {
    ...
    // Функция - обработчик проверяет массив строк на корректность и создает класс
    // FioDate с данными пользователя (порядок следования данных неважен)
    public FioDate handler(String[] arrayOfString) throws DateIncorrectException {
        ...
        if (fioFlag == 3 && dateOfBirthFlag && telephoneFlag && genderFlag)
            return new FioDate(fio[0], fio[1], fio[2], dateOfBirth, numberTelephone, gender);
        else {
            String messageOfError = "Внимание!Введенные данные некоректны!";
            if (fioFlag < 3)
                messageOfError = messageOfError + "Некоректные данные Ф.И.О.";
            if (!dateOfBirthFlag)
                messageOfError = messageOfError + "Некоректная дата рождения";
            if (!telephoneFlag)
                messageOfError = messageOfError + "Некоректный телефонный номер";
            if (!genderFlag)
                messageOfError = messageOfError + "Некоректный пол f/m ";
